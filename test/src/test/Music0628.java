...
class MusicManager{
	// 멤버변수 => MusicManager가 관리는 변수 
	_Music[] music=new _Music[50]; // null
	
	// 초기화 => 생성자 (멤버초기화 역할)
	MusicManager(){
		for(int i=0;i<50;i++){
			music[i]=new _Music(); // 메모리 할당 => 메모리를 생성 
	...
	void musicListData(){
		for(_Music m:music){
			String s="";
			if(m.state.equals("상승"))	s="▲ ("+m.modify+")";
			else if(m.state.equals("하강"))	s="▽ ("+m.modify+")";
			else	s="─";
